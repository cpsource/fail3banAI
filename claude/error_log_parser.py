...
def parse_error_log(log_line: str) -> Dict[str, Optional[str]]:
    """
...
def _decode_url_encoded_string(text: str) -> str:
    ...
    The pattern %%32%65 is malformed and should be treated as %32%65:
    - %%32%65 → %32%65 (fix malformed double %) 
    - %32%65 → 2e (hex 32='2', hex 65='e')
    - 2e → . (hex 2e = ASCII dot)
    - So %%32%65%%32%65 → 2e2e → .. (directory traversal)
    ...
    try:
        ...
        # Pattern like "2e2e" should become ".."
        # This handles cases where hex values are adjacent without % separators
        decoded = _decode_adjacent_hex_pairs(decoded)
...
def _decode_adjacent_hex_pairs(text: str) -> str:
    """
    Decode adjacent hex pairs like '2e2e' to ASCII characters.
    
    This is like having a hex-to-ASCII translator for cases where
    attackers use raw hex values without proper % encoding.
    """
    import re
    
    # Find sequences of hex digit pairs (like 2e2e2e)
    # Look for even-length sequences of hex digits
    ...
    # Match sequences of 4 or more hex digits (at least 2 pairs)
    # This avoids false positives with single hex digits
    pattern = r'\b([0-9a-fA-F]{4,})\b'
    
    # Only process if it looks like a hex sequence in a path context
    if re.search(r'/[0-9a-fA-F]{4,}/', text):
        decoded = re.sub(pattern, hex_replacer, text)
        return decoded
...
if __name__ == "__main__":
    ...
    for i, log_line in enumerate(test_logs, 1):
        result = parse_error_log(log_line)

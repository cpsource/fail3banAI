...
class Parselet_OPTIONS:
    ...
    def compress_line(self, log_line):
        ...
        pattern = r' - - \[(.*?)\] "(OPTIONS) (/.*?) HTTP/\d\.\d" (\d{3}) (\d+) ".*?" "(.*?)"'
        ...
        if match:
            ...
            http_method = match.group(2)       # Extract HTTP method (OPTIONS)
...
if __name__ == "__main__":
    ...
    log_line = '64.225.75.246 - - [28/Sep/2024:00:31:27 +0000] "OPTIONS /.env HTTP/1.1" 302 841 "-" "Go-http-client/1.1"'

    # Create an instance of the Parselet_OPTIONS class
